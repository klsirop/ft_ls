...
/*   Updated: 2020/11/17 16:22:04 by volyvar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tree	*ft_insert_tree_element(t_tree *head, t_file_info *new, char *parent_name, enum e_order_type order_type) {
	...
	if (head == NULL) {
		...
		is_right_order = ft_is_right_order(parent_name, head->field->file_name, new->file_name, order_type);
		if (is_right_order == 0) {
			head->left = ft_insert_tree_element(head->left, new, parent_name, order_type);
		} else {
			head->right = ft_insert_tree_element(head->right, new, parent_name, order_type);
		}
	}
	return head;
}
